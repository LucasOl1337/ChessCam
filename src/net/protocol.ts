...
export type ClientMessage =
  ...
  | { type: 'webrtc-offer' | 'webrtc-answer' | 'ice-candidate'; payload: any };

export type ServerMessage =
  | { type: 'room-joined'; roomCode: string; color: 'white' | 'black'; opponent?: { id: string } }
  | { type: 'move-made'; move: any; fen: string; lastMove: string }
  | { type: 'game-over'; reason: 'checkmate' | 'stalemate' | 'draw' | 'resignation'; winner?: 'white' | 'black' }
  | { type: 'webrtc-offer' | 'webrtc-answer' | 'ice-candidate'; from: string; payload: any }
